...
library(ggplot2)
library(ggrepel)  # 用于更好地显示标签

p <- ggplot(pca.data, aes(x = PC1, y = PC2, fill = Type, label = sample)) +  # 假设你的数据框中有一个 Name 列
  stat_centroid(aes(xend = PC1, yend = PC2, colour = Type),
  ...
  geom_label_repel(show.legend = FALSE, size = 4, box.padding = 0.5)  # 添加标签，避免重叠
...
#?useMart
mart_oas=useMart("ensembl", "hsapiens_gene_ensembl")#mmusculus   hsapiens_gene_ensembl
